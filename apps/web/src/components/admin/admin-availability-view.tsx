...
import type { AdminPropertyRow, OwnerAvailabilitySlotRow } from '@mazare3/shared';
...
export function AdminAvailabilityView() {
  ...
  const loadSlots = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchAdminAvailability(propertyId, from, to);
      setSlots(res.data);
  ...
  useEffect(() => {
    if (propertyId) {
      router.replace(`/admin/availability?propertyId=${propertyId}`);
    }
  }, [propertyId, router]);
  ...
  async function toggleBlock(slot: OwnerAvailabilitySlotRow) {
    ...
    try {
      ...
      await patchAdminAvailabilitySlot(slot.id, { status: nextStatus });
      await loadSlots();
    } catch (e) {
      if (e instanceof AdminApiError && e.code === 'SLOT_HAS_ACTIVE_BOOKING') {
        setError(t('slotBookedError'));
      } else {
  ...
  async function savePrice(slot: OwnerAvailabilitySlotRow) {
    ...
    setError(null);
    try {
      await patchAdminAvailabilitySlot(slot.id, { price });
      await loadSlots();
    } catch (e) {
      if (e instanceof AdminApiError && e.code === 'SLOT_HAS_ACTIVE_BOOKING') {
        setError(t('slotBookedError'));
  ...
  if (loading) {
    return (
      <div data-testid="admin-availability" className="flex justify-center py-20">
  ...
  const propertyTitle =
    selectedProperty && (locale === 'ar' ? selectedProperty.titleAr : selectedProperty.titleEn);
      ...
      <Card className="glass-panel rounded-2xl border-primary/12">
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-2 sm:col-span-2">
            ...
            <select
              data-testid="admin-availability-property"
