# Mazare3 — Privacy Policy Phase 3C.4B.2A Review Export

**Status:** DRAFT only — NOT counsel-activated — NOT Production-active  
**Document type:** privacy_policy  
**Version:** 1.1.0-advisor-revised  
**Historical corpus preserved:** 1.0.1-launch-candidate (`privacy-policy-launch-candidate.ts`)  
...
| Version string | `1.1.0-advisor-revised` |
| Activation | DRAFT / non-active |
| dpoAppointed | `false` (must remain false until formal appointment) |
| Privacy DPO readiness | `NOT_FORMALLY_RECORDED` / public contact `NOT_PUBLISHED` |
...
```markdown
> للمراجعة الداخلية فقط — مسودة سياسة الخصوصية المنقّحة للمستشار (1.1.0). مبنية على أساس الخصوصية في مزارع (3C.4B.1–3C.4B.1.5). غير مفعّلة من المستشار القانوني. غير مفعّلة في الإنتاج.

# سياسة الخصوصية

توضّح سياسة الخصوصية هذه كيفية معالجة البيانات الشخصية في إطار سوق مزارع (مزارع / مزارع الأردن)، الذي تشغّله [[LEGAL_ENTITY_INTRO_AR]]. هذه الوثيقة إشعار شفافية. الإقرار بسياسة الخصوصية لا يُعدّ موافقة مسبقة، وهو منفصل عن قبول الشروط والأحكام وعن أي خيارات اختيارية للتسويق أو التحليلات.
...
بريد التواصل للخصوصية (الصندوق القابل للنشر): [[PRIVACY_CONTACT_EMAIL]]. إلى أن يُحسم هذا التواصل ويُنشر، يمكن للأشخاص المعنيين استخدام أدوات الخصوصية داخل الحساب حيث تتوفر.

حيث يُشترط تعيين مسؤول لحماية البيانات الشخصية ويُعيَّن رسمياً وفق المتطلبات الأردنية المنطبقة، ستنشر مزارع تفاصيل التواصل الخاصة به هنا ([[DPO_OR_PRIVACY_CONTACT]]). ولا تدّعي مزارع حالياً أن مسؤولاً لحماية البيانات قد عُيّن أو اعتُمد رسمياً بالفعل.


## 3. فئات البيانات الشخصية — الزبائن
...
## 4. فئات البيانات الشخصية — المالكون / الشركاء
...
## 6. كيف تجمع مزارع البيانات الشخصية
...
## 8. الأساس القانوني والموافقة المسبقة
...
## 9. مدة الموافقة والسحب وإعادة الموافقة
...
## 10. مشاركة البيانات والمستلمون والمعالجون
...
| المزود / الخدمة | الغرض (ملخص) | فئات البيانات (ملخص) | منطقة المعالجة / التخزين | ملاحظة عبر الحدود |
| --- | --- | --- | --- | --- |
| مزود قاعدة البيانات الأساسي | تخزين بيانات التطبيق وتشغيله | الحساب والحجز وبيانات الدفع الوصفية وأدلة الموافقة والقبول القانوني وسجلات الدعم (حسب الاقتضاء) | [[PROCESSOR_REGION_NEON]] | المنطقة غير مؤكدة علناً حتى الحسم |
| مزود تخزين الكائنات | وسائط الإعلان وملفات تحقق المالك المقيّدة | ملفات الوسائط؛ مستندات التحقق/الصلاحية (مقيّدة) | [[PROCESSOR_REGION_R2]] | المنطقة غير مؤكدة علناً حتى الحسم |
| مزود خدمة الدفع المهيأ ([[PAYMENT_PROVIDER_LEGAL_NAME]]) | معالجة البطاقة ونتائج الدفع | بيانات دفع وصفية؛ بيانات البطاقة لدى المزود (لا تخزّن مزارع الرقم الكامل/رمز الأمان) | وفق تهيئة المزود | تُقيَّم وفق قواعد النقل الأردنية عند تأكيد المنطقة |
...
| مزود استضافة التطبيق ([[APP_HOSTING_PROVIDER]]) | استضافة التطبيق | حركة تشغيلية وتطبيقية حسب الاستضافة | [[APP_HOSTING_REGION]] | هوية/منطقة الاستضافة غير محسومة حتى التأكيد |
...
ولا تدّعي مزارع أن جميع البيانات الشخصية تبقى حصراً داخل الأردن. وقد يعالج بعض المعالجين أو مزودي الخدمات البيانات الشخصية أو يخزّنونها خارج الأردن. وتبقى المناطق الدقيقة لبعض المزودين غير محسومة في هذه المسودة ([[PROCESSOR_REGION_NEON]]، [[PROCESSOR_REGION_R2]]، [[APP_HOSTING_PROVIDER]]، [[APP_HOSTING_REGION]]).
...
| طلبات الخصوصية / الشكاوى وسجلات الانتهاك | [[RETENTION_BREACH]] |
...
يمكنك تقديم طلبات الخصوصية عبر أدوات الخصوصية داخل الحساب حيث تتوفر، وعبر تواصل الخصوصية القابل للنشر بعد حسمه ([[PRIVACY_CONTACT_EMAIL]]).
...
## 19. ملفات تعريف الارتباط والتقنيات المماثلة
...
## 21. الأطفال ومن ليس لديهم أهلية قانونية
...
المسؤول عن المعالجة: [[LEGAL_ENTITY_INTRO_AR]]. العنوان المسجّل: [[REGISTERED_ADDRESS_AR]]. تواصل الخصوصية: [[PRIVACY_CONTACT_EMAIL]]. الوثائق ذات الصلة: الشروط والأحكام، وسياسة ملفات تعريف الارتباط، وأدوات الخصوصية داخل الحساب حيث تتوفر.
```
...
```markdown
> INTERNAL REVIEW ONLY — Privacy Policy advisor-revised DRAFT (1.1.0). Based on Mazare3 privacy foundation 3C.4B.1–3C.4B.1.5. Not counsel-activated. Not Production-active.

# Privacy Policy

This Privacy Policy explains how Personal Data is processed in connection with the Mazare3 marketplace (Mazare3 / Mazare3 Jordan / مزارع الأردن), operated by [[LEGAL_ENTITY_INTRO_EN]]. This document is a transparency notice. Acknowledging this Privacy Policy is not Prior Consent, and is separate from accepting the Terms & Conditions and from any optional marketing or analytics choices.
...
Privacy contact email (publishable mailbox): [[PRIVACY_CONTACT_EMAIL]]. Until this contact is resolved and published, Data Subjects may use in-account privacy tools where available.

Where an appointed Data Protection Officer is required and formally designated under applicable Jordanian requirements, Mazare3 will publish the applicable DPO contact details here ([[DPO_OR_PRIVACY_CONTACT]]). Mazare3 does not currently claim that a Data Protection Officer has already been formally appointed or accredited.


## 3. Categories of Personal Data — Customers
...
## 4. Categories of Personal Data — Owners / Partners
...
## 6. How Mazare3 collects Personal Data
...
## 8. Legal basis and Prior Consent
...
## 9. Consent duration, withdrawal, and re-consent
...
## 10. Data sharing, recipients, and processors
...
| Provider / service | Purpose (summary) | Data categories (summary) | Processing / storage region | Cross-border note |
| --- | --- | --- | --- | --- |
| Primary database provider | Application data storage and operation | Account, Booking, payment metadata, consent and legal evidence, support records (as applicable) | [[PROCESSOR_REGION_NEON]] | Region not publicly confirmed until resolved |
| Object-storage provider | Listing media and restricted Owner verification files | Media files; KYC/authority documents (restricted) | [[PROCESSOR_REGION_R2]] | Region not publicly confirmed until resolved |
| Configured payment service provider ([[PAYMENT_PROVIDER_LEGAL_NAME]]) | Card processing and payment outcomes | Payment metadata; card data handled by provider (Mazare3 does not store PAN/CVV) | As configured with the provider | Assessed under Jordanian transfer rules when region confirmed |
...
| Application hosting provider ([[APP_HOSTING_PROVIDER]]) | Hosting the application | Operational and application traffic as hosted | [[APP_HOSTING_REGION]] | Hosting identity/region unresolved until confirmed |
...
Mazare3 does not claim that all Personal Data remains exclusively in Jordan. Some processors or service providers may process or store Personal Data outside Jordan. Exact regions for certain providers remain unresolved in this DRAFT ([[PROCESSOR_REGION_NEON]], [[PROCESSOR_REGION_R2]], [[APP_HOSTING_PROVIDER]], [[APP_HOSTING_REGION]]).
...
| Category | Retention rule (pending approval) |
| --- | --- |
...
| Privacy requests / complaints and breach records | [[RETENTION_BREACH]] |
...
You may submit privacy requests through in-account privacy tools where available, and through the publishable privacy contact once resolved ([[PRIVACY_CONTACT_EMAIL]]).
...
## 21. Children and persons without legal capacity
...
## 22. Policy updates and versioning
...
Controller: [[LEGAL_ENTITY_INTRO_EN]]. Registered address: [[REGISTERED_ADDRESS_EN]]. Privacy contact: [[PRIVACY_CONTACT_EMAIL]]. Related documents: Terms & Conditions, Cookie Policy, and in-account privacy tools where available.
```
