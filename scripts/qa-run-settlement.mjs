/**
 * Focused 10D.1 runner: settlements + payout operations + payment integrity.
 */
...
const QA_PORT = process.env.QA_API_PORT ?? '4016';
const QA_BASE = `http://localhost:${QA_PORT}/api/v1`;
const USE_SHELL = process.platform === 'win32';

const SCRIPTS = ['qa-settlement-cycle-api.mjs', 'qa-settlement-api.mjs'];
...
async function main() {
  ...
  try {
    ...
    await runScript('qa-payment-integrity.mjs');
    console.log('\n✅ Focused settlement-cycle + settlement + integrity tests passed\n');
  } catch (e) {
