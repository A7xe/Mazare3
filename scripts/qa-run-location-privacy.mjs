/**
 * Focused 10H.2A runner: location privacy math + API.
 */
...
const QA_PORT = process.env.QA_API_PORT ?? '4022';
...
function runMath() {
  return new Promise((resolve, reject) => {
    const child = spawn('pnpm', ['exec', 'tsx', 'scripts/qa-location-privacy-math.ts'], {
      cwd: API_DIR,
      shell: USE_SHELL,
      stdio: 'inherit',
    });
    ...
    child.on('exit', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`qa-location-privacy-math.ts exited with code ${code}`));
    });
  });
}

async function main() {
  console.log('\n── qa-location-privacy-math.ts ──\n');
  await runMath();
  ...
  try {
    ...
    console.log('\n── qa-location-privacy-api.mjs ──\n');
    await runNode('scripts/qa-location-privacy-api.mjs');
    console.log('\n✅ Focused location/privacy tests passed\n');
  } catch (e) {
