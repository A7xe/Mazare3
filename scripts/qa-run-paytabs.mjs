/**
 * Focused 10G.2A runner: PayTabs adapter (mocked) + gateway/deposit/payment/integrity regressions.
 */
...
const QA_PORT = process.env.QA_API_PORT ?? '4036';
const QA_BASE = `http://localhost:${QA_PORT}/api/v1`;
const USE_SHELL = process.platform === 'win32';

const SCRIPTS = [
  'qa-paytabs-adapter.mjs',
  'qa-payment-gateway-boundary-api.mjs',
  'qa-deposit-api.mjs',
  'qa-payment-api.mjs',
...
async function main() {
  ...
  try {
    console.log('\n── qa-paytabs-adapter.mjs (no API) ──\n');
    await runScript('qa-paytabs-adapter.mjs');
    ...
    console.log('\n✅ Focused PayTabs adapter + payment regressions passed');
    console.log('Live PayTabs verification: NOT_RUN (no real credentials/network)\n');
  } catch (e) {
