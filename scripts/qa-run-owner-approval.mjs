/**
 * Focused 10C.2A runner: owner-approval QA + deposit regression.
 */
...
const QA_PORT = process.env.QA_API_PORT ?? '4015';
const QA_BASE = `http://localhost:${QA_PORT}/api/v1`;
const USE_SHELL = process.platform === 'win32';

const SCRIPTS = [
  'qa-owner-approval-api.mjs',
  'qa-owner-approval-expiry-api.mjs',
  'qa-owner-inbox-api.mjs',
  'qa-owner-performance-api.mjs',
];
...
async function main() {
  ...
  try {
    ...
    console.log('\n── qa-owner-performance-math.mjs ──\n');
    await runScript('qa-owner-performance-math.mjs');
    for (const script of SCRIPTS) {
      console.log(`\n── ${script} ──\n`);
      await runScript(script);
    }
    console.log('\n✅ Focused owner-approval + inbox tests passed\n');
  } catch (e) {
